...
/**
 * The type Spotify feign error decoder.
 */
@RequiredArgsConstructor
public class SpotifyFeignErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String s, Response response) {
        logging.error(() -> "Spotify Client Response Error: Got response " + response.status() + " from " + s + " with body: " + response.body());

        switch (response.status()) {
            case 404:
                throw new AppException(ErrorCodesEnum.PLAYLIST_NOT_FOUND);
            case 400:
                throw new AppException(ErrorCodesEnum.INTERNAL_ERROR, "Bad request for Spotify API");
            case 401:
                throw new AppException(ErrorCodesEnum.INTERNAL_ERROR, "Unauthorized to access Spotify API");
            default:
